...
int CReferenceWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	{
		//m_wndPDF.m_hWnd = NULL;
		MessageBox(_T("需要安装 ADOBE READER ，否则无法使用参考此项功能"));
		return 0;
	}
...
BOOL CReferenceWnd::OpenReferenceList(CString csName)
{
	...
	if (bRet)
		...
		csFileName.Empty();
	m_wndPDF.setShowToolbar(FALSE);

	m_wndPDF.setLayoutMode(_T("SinglePage"));
	m_wndPDF.setPageMode(_T("none"));
	//m_wndPDF.SetScrollRange(SB_VERT,0,10000,TRUE);
...
int CReferenceWnd::GetDFName(CString &csRoute)
{
	...
	if (_OpenCSVFile(csDIR+_T("\\Reference\\DFName.csv"),&CSFFile) != TRUE)
	{
		MessageBox(_T("打开DF name file 失败"));
		return FALSE;
	}
...
int CReferenceWnd::GetReferenceInformation(CString csRoute, CString csFID, CString& csDOC, long* ulPage, unsigned int* uiPosition)
{
	...
	if (!__GetCurrentDIR(csTemp))
		return FALSE;
	if (_OpenCSVFile(csTemp+ _T("\\Reference\\ReferenceList.csv"),&CSFFile) != TRUE)
	{
		MessageBox(_T("打开Reference list file 失败"));
		return FALSE;
	}
...
int CReferenceWnd::GetReferencecsSpecificationName(CString csDOC,CString&csSpecificationName)
{
	...
	if (_OpenCSVFile(csTemp+ _T("\\Reference\\SpecificationList.csv"),&CSFFile) != TRUE)
	{
		MessageBox(_T("打开Specification List file 失败"));
		return FALSE;
	}
