...
unsigned long crc32buf(unsigned char *buf, unsigned int len)
{
...
{
	return crc32buf(pucInput, uiInputDataLen);
}
/*
CRYPTO_API BOOL crc_32(unsigned char * pucInput, int uiInputDataLen, unsigned char ucOutput[4])
{
	unsigned long ulCrc;
	int i;
	ulCrc = crc32buf((char *)pucInput, uiInputDataLen);
	for(i = 0; i<4; i++)
	{
    	ucOutput[3-i] = (ulCrc >> 8*i) & 0xff;
	}
	return TRUE;
}
*/
